...
<head>
    ...
    <script type="text/javascript">
        $(function () {
            //div 两个哪个显示呢？
            if (null != "${sessionScope.user.email}" && "${sessionScope.user.email}" != "") {
            ...
            $.post("${pageContext.request.contextPath}/afterVideo/updatePlayNum", params);
        });
    </script>
</head>
...
<div>
    ...
    <div class="catalog">
        <div class="container">
            ...
            <c:forEach items="${video.course.videoList}" var="video1">
                <div class="chapter" onclick="load(${video1.id})">
                    <p class="biaoti"><a
                            href="${pageContext.request.contextPath}/afterVideo/showVideo?id=${video1.id}&subjectName=${subjectName}">${video1.title}</a></p>
                    <p class="lecturer">${video1.detail}</p>
                    <p class="lecturer">讲师：${video1.speaker.speakerName}</p>
                    <div class="v-info">
                        <span class="count"><img src="${pageContext.request.contextPath}/img/count.png"
                                                 alt="">${video1.playNum}</span>
                        <span class="duration"><img src="${pageContext.request.contextPath}/img/player.png"
                                                    alt="">${video1.time}</span>
                    </div>
                </div>
            </c:forEach>
